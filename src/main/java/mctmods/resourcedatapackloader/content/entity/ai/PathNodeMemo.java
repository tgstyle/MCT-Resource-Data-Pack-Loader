...
public final class PathNodeMemo {
    private static final PathType[] KINDS = PathType.values();
    ...
    @Nullable public PathType known(int owner, int world, long tick, int x, int y, int z) {
    ...
    public void remember(int owner, int world, long tick, int x, int y, int z, PathType kind) {
        long key = packed(x, y, z);
        int slot = slot(key, world, owner);
        where[slot] = key;
        when[slot] = tick;
