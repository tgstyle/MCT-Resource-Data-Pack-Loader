...
public final class ContentChunkWatch {
    ...
    @SubscribeEvent public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        ContentLog.LOGGER.debug(String.format(
                "It also lit %d chunk(s) in %.1f ms, of which %d were left dark for want of their neighbours %d were the same chunks being tried again and %d were held back until the dressing was on and %d chunk(s) were left undressed while only the light was being seen to, went back over %d chunk(s) in %.1f ms to finish the job, and spread light from %d place(s), %d about the sky and %d about lamps and fire, taking about %.1f ms in all of which about %.1f ms was the sky, timed one in every %d. %d more were not asked at all, the sky about them being settled already",
