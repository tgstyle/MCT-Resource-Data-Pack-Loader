...
public final class ContentGeode implements IContentShape {
    ...
    public ContentGeode(ShapeDef shape, BlockState outline, @Nullable BlockState fill) {
        this.shape = shape;
    ...
    @Override public boolean generate(ContentPlacer placer, RandomSource random, BlockPos origin) {
        ...
        if (blocked(placer, baseX, baseY, baseZ, width, height, body, hollow)) { return false; }
        ...
        for (int x = 0; x < width; x++) {
            for (int z = 0; z < width; z++) {
                for (int y = 0; y < height; y++) {
                    int at = index(x, z, y, width, height);
                    if (fill != null && hollow[at]) { placed |= placer.placeExactly(fill, baseX + x, baseY + y, baseZ + z); }
                    else if (!body[at] && touches(body, x, z, y, width, height)) { placed |= placer.placeExactly(outline, baseX + x, baseY + y, baseZ + z); }
                }
            }
        }
        return placed;
    ...
    private boolean blocked(ContentPlacer placer, int baseX, int baseY, int baseZ, int width, int height, boolean[] body, boolean[] hollow) {
        for (int x = 0; x < width; x++) {
            for (int z = 0; z < width; z++) {
                for (int y = 0; y < height; y++) {
                    int at = index(x, z, y, width, height);
