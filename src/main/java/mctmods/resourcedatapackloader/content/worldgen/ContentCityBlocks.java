...
public final class ContentCityBlocks extends StructureProcessor {
    private static final StructureProcessorType<ContentCityBlocks> TYPE = () -> Codec.unit(new ContentCityBlocks(100));
    ...
    private static void readRules() {
        ...
        for (String entry : ContentControl.list(ContentControl.STRUCTURES, "villageBlocks", Config.worldgen.villageBlocks())) {
            ...
            Block found = Registered.find(ForgeRegistries.BLOCKS, ResourceLocation.tryParse(rest));
            if (found == null) {
            ...
            RULES.put(from, new Rule(found.defaultBlockState(), Math.max(1, Math.min(100, chance))));
        }
    }
