...
public final class BeardRoads {
    ...
    public static void pave(StructureComponent piece, World world, StructureBoundingBox clip, IBlockState path, IBlockState gravel, IBlockState planks, boolean chosenSurface) {
        ...
        DeadEndCap cap = deadEndCap(world, nearby, piece, alongX);
            ...
            for (int across = acrossLeast; across <= acrossMost; across++) {
                ...
                if (profile[i] == Integer.MIN_VALUE) {
                    ...
                    if (wet) {
                        ...
                        paved += deckBridge(world, box, alongX, start + i, across, acrossLeast, acrossMost, deckAt, planks, gravel, at, dock, crossed, legs[i]);
                ...
                if (profile[i] != Integer.MIN_VALUE) {
                    ...
                    if (bridged[i]) {
                        paved += deckBridge(world, box, alongX, start + i, across, acrossLeast, acrossMost, profile[i], planks, gravel, at, dock, crossed, legs[i]);
                        if (dock == null) { paved += deadEndCap(cap, world, piece, alongX, start + i, across, acrossLeast, at.getY(), at); }
                        continue;
                    }
                    if (wet && !pier) {
                        ...
                        if (!BeardKeep.holds(x, profile[i], z)) {
                            ...
                            paved++;
                            if (joint) { paved += deckRail(world, box, alongX, start + i, across, acrossLeast, acrossMost, profile[i], planks, gravel, at, dock, crossed, false); }
                        }
                        continue;
                    }
                }
                ...
                if (profile[i] == Integer.MIN_VALUE) {
                    ...
                    paved += deckRail(world, box, alongX, start + i, across, acrossLeast, acrossMost, target, planks, gravel, at, dock, crossed, legs[i]);
                ...
                if (joint) { paved += deckRail(world, box, alongX, start + i, across, acrossLeast, acrossMost, target, planks, gravel, at, dock, crossed, false); }
            }
    ...
    @Nullable private static Pier pierFor(World world, StructureComponent piece, boolean alongX, StructureBoundingBox box, Grade graded) {
        if (roadNarrow(box, alongX)) { return null; }
    ...
    private static int bulbVerge(World world, StructureComponent piece, Bulb bulb, StructureBoundingBox clip, BlockPos.MutableBlockPos at) {
        ...
        for (int z = Math.max(box.minZ - verge, clip.minZ); z <= Math.min(box.maxZ + verge, clip.maxZ); z++) {
            for (int x = Math.max(box.minX - verge, clip.minX); x <= Math.min(box.maxX + verge, clip.maxX); x++) {
                ...
                filled += vergeFill(world, piece, x, z, bulb.level, at);
            }
        }
        return filled;
    }

    private static int tunnelWall(World world, StructureComponent piece, BlockPos.MutableBlockPos at, int x, int z, int level, IBlockState lining) {
    ...
    private static void dropShortRuns(boolean[] tunnels) {
        int i = 0;
    ...
    private static boolean[] tunnelLights(boolean[] tunnels, int start, int run) {
    ...
    private static int roofCell(World world, BlockPos.MutableBlockPos at, int x, int z, int y, IBlockState block) {
    ...
    private static boolean crossedRow(List<StructureBoundingBox> crossed, boolean alongX, int row) {
        for (StructureBoundingBox other : crossed) {
            if (row >= (alongX ? other.minX : other.minZ) - 1 && row <= (alongX ? other.maxX : other.maxZ) + 1) { return true; }
        }
        return false;
    }
    ...
    private static IBlockState tunnelBlock() { return pathBlock("villagePathTunnelBlock", Config.worldgen.villagePathTunnelBlock, Blocks.AIR.getDefaultState()); }

    public static boolean tunnelAt(@Nullable Grade grade, int row) { return grade != null && grade.tunneledAt(row, tunnelDepth()); }
    ...
    private static List<StructureComponent> villagePieces(World world) { return ContentBeard.everyone(world, ContentBeard.components()); }
    ...
    private static boolean onPiling(int row, int across, int acrossLeast, int acrossMost) {
    ...
    private static boolean[] bridgeLegs(int[] profile, boolean[] bridged) {
    ...
    private static boolean overDryDrop(World world, BlockPos.MutableBlockPos at, int x, int deckY, int z) {
        for (int y = deckY - 1; y >= deckY - PILING_REACH && y >= 1; y--) {
            ...
            if (held.getMaterial().isSolid()) { return deckY - y > FILL_UNDER; }
        }
        return false;
    }
    ...
    private static int piling(World world, boolean alongX, int row, int across, int fromY, IBlockState support, BlockPos.MutableBlockPos at) {
        int laid = 0;
        for (int y = fromY; y >= fromY - PILING_REACH && y >= 1; y--) {
    ...
    public static int deckBridge(World world, StructureBoundingBox box, boolean alongX, int row, int across, int acrossLeast, int acrossMost, int deckAt, IBlockState planks, IBlockState support, BlockPos.MutableBlockPos at, @Nullable Pier dock, List<StructureBoundingBox> crossed, boolean leg) {
        int deckY = deckAt;
        ...
        int laid = deckRail(world, box, alongX, row, across, acrossLeast, acrossMost, deckY, planks, support, at, dock, crossed, leg);
    ...
    @Nullable private static IBlockState deckState(World world, StructureBoundingBox box, boolean alongX, int row, int across, int acrossLeast, int acrossMost, IBlockState planks, @Nullable Pier dock, List<StructureBoundingBox> crossed) {
        ...
        if (mine.oversize || mine.width < 3 || pathFullWidth() == 3) { return planks; }
        int x = alongX ? row : across;
        int z = alongX ? across : row;
        if (!mine.alley) {
            IBlockState stamped = stampAt(world, box, alongX, row, across, acrossCenter, mine.core, planks, planks, crossed);
            if (stamped != null) { return stamped; }
            IBlockState shared = overlapCell(world, box, alongX, row, across, mine.core, planks, planks, crossed);
        ...
        if (role == BeardCross.LINE) { return ContentBeard.axised(pathBlock("villagePathLineBlock", Config.worldgen.villagePathLineBlock, planks), top.alongX); }
        if (top.alley || !top.middle(x, z) || BeardCross.crossedOver(shapes, top.alongX, x, z)) { return planks; }
    ...
    private static int deckRail(World world, StructureBoundingBox box, boolean alongX, int row, int across, int acrossLeast, int acrossMost, int deckY, IBlockState planks, IBlockState support, BlockPos.MutableBlockPos at, @Nullable Pier dock, List<StructureBoundingBox> crossed, boolean leg) {
        if (roadNarrow(box, alongX) && squareAt(box, alongX, crossed, alongX ? row : across, alongX ? across : row)) { return 0; }
        ...
        int laid = 0;
        if (leg && overDryDrop(world, at, alongX ? row : across, deckY, alongX ? across : row)) { laid += piling(world, alongX, row, across, deckY - 1, support, at); }
        IBlockState barrier = pathBlock("villagePathBridgeBarrierBlock", Config.worldgen.villagePathBridgeBarrierBlock, planks);
        if (barrier != planks) { laid += raise(world, barrier, alongX ? row : across, deckY, alongX ? across : row, at); }
        at.setPos(alongX ? row : across, deckY, alongX ? across : row);
        return laid;
    }

    private static List<BeardCross> shapesAt(StructureBoundingBox box, boolean alongX, List<StructureBoundingBox> crossed) {
    ...
    private static StructureBoundingBox through(StructureBoundingBox road, boolean roadAlongX, StructureBoundingBox box) {
        ...
        int coreHalf = (pathFullWidth() - 1) / 2 - pathLineColumns() - pathSidewalkWidth();
        if (roadAlongX) {
    ...
    @Nullable public static IBlockState dressSurface(World world, StructureComponent piece, boolean alongX, int row, int across, int acrossCenter, IBlockState path, IBlockState planks, List<StructureBoundingBox> crossed) {
        StructureBoundingBox box = piece.getBoundingBox();
        BeardCross mine = BeardCross.of(box, alongX);
        if (mine.oversize || mine.width < 3 || pathFullWidth() == 3) { return null; }
        int x = alongX ? row : across;
        int z = alongX ? across : row;
        if (!mine.alley) {
            IBlockState stamped = stampAt(world, box, alongX, row, across, acrossCenter, mine.core, path, planks, crossed);
            if (stamped != null) { return stamped; }
        ...
        if (role == BeardCross.LINE) { return ContentBeard.axised(pathBlock("villagePathLineBlock", Config.worldgen.villagePathLineBlock, path), top.alongX); }
        IBlockState surface = top.alley ? alleyBlock(path) : path;
        if (top.alley || !top.middle(x, z) || BeardCross.crossedOver(shapes, top.alongX, x, z)) { return surface; }
        IBlockState center = pathBlock("villagePathCenterBlock", Config.worldgen.villagePathCenterBlock, surface);
    ...
    private static int arms(StructureBoundingBox ew, StructureBoundingBox ns, StructureBoundingBox mine, List<StructureBoundingBox> crossed) {
        ...
        for (int i = -1; i < crossed.size(); i++) {
            ...
            if (roadNarrow(c, BeardPlots.roadAlongX(c))) { continue; }
    ...
    private static boolean squareAt(StructureBoundingBox box, boolean alongX, List<StructureBoundingBox> crossed, int x, int z) {
        for (StructureBoundingBox road : crossed) {
            if (BeardPlots.roadAlongX(road) == alongX) { continue; }
            if (!roadNarrow(box, alongX) && roadNarrow(road, !alongX)) { continue; }
    ...
    @Nullable private static IBlockState junctionCell(World world, boolean alongX, int row, int across, int acrossCenter, int core, StructureBoundingBox ew, StructureBoundingBox ns, StructureBoundingBox mine, List<StructureBoundingBox> crossed, IBlockState road0, IBlockState planks) {
        ...
        int otherCore = roadCore(alongX ? ns : ew, !alongX);
    ...
    public static void roadApron(World world, @Nullable StructureComponent piece, boolean alongX, int start, int rowMost, int acrossLeast, int acrossMost, int[] profile, int[] ground, boolean[] held, boolean[] footed, boolean[] fixed, boolean[] bridged, boolean[] square) {
        ...
        for (int[] flat : flats) {
            ...
            int grade = reachable(profile, held, start, rowMost, otherLeast, otherMost, flat[2]);
    ...
    @Nullable static DeadEndCap deadEndCap(World world, List<StructureComponent> nearby, StructureComponent piece, boolean alongX) {
        ...
        for (String asked : wanted) {
            if ("sidewalk".equals(asked) && walk.getBlock() != Blocks.AIR && !roadNarrow(piece.getBoundingBox(), alongX)) { usable.add(asked); }
    ...
    private static int railEdge(int span) { return Math.min(1 + pathExtraWidth() + 1, (span - 1) / 2); }

    private static int barrierHeight() { return Math.max(1, ContentControl.number(ContentControl.VILLAGES, "villagePathBridgeBarrierHeight", Config.worldgen.villagePathBridgeBarrierHeight)); }

    private static int raise(World world, IBlockState barrier, int x, int deckY, int z, BlockPos.MutableBlockPos at) {
        int laid = 0;
        for (int y = deckY + 1; y <= deckY + barrierHeight(); y++) {
            at.setPos(x, y, z);
            if (world.getBlockState(at).getMaterial().isSolid()) { break; }
