    ...
    @SubscribeEvent public static void onDecorated(PopulateChunkEvent.Post event) {
        ...
        for (StructureStart start : starts) {
            ...
            for (StructureComponent piece : start.getComponents()) {
                if (piece instanceof StructureVillagePieces.Path) { alongRoad(world, piece, clip); }
            }
        }
    }

    private static void alongRoad(World world, StructureComponent piece, StructureBoundingBox clip) {
        ...
        for (int row = from; row <= to; row++) {
            ...
            place(world, clip, at, alongX ? row : near, alongX ? near : row);
            place(world, clip, at, alongX ? row : far, alongX ? far : row);
        }
    }

    private static void place(World world, StructureBoundingBox clip, BlockPos.MutableBlockPos at, int x, int z) {
