...
public final class PortalShapes {
    ...
    public static boolean roomy(List<String> shape, PortalFrameDef frame, boolean flat) { return roomy(shape, frame, flat ? PortalFrameDef.LEAST_WIDE : PortalFrameDef.LEAST_TALL); }

    private static boolean roomy(List<String> shape, PortalFrameDef frame, int leastTall) {
        int[] span = hole(shape);
        if (span == null) { return false; }
        return span[0] >= PortalFrameDef.LEAST_WIDE && span[1] >= leastTall && span[0] <= frame.maxWidth() && span[1] <= frame.maxHeight();
    }
    ...
    @Nullable private static List<String> grow(List<String> rows, int tall, int wide) {
    ...
    public static BlockPos at(BlockPos origin, int column, int row, int rows, boolean alongX, boolean flat, boolean mirrored) {
        ...
        if (flat) { return alongX ? origin.offset(across, 0, row) : origin.offset(row, 0, across); }
        int up = rows - 1 - row;
        return alongX ? origin.offset(across, up, 0) : origin.offset(0, up, across);
    }
}
