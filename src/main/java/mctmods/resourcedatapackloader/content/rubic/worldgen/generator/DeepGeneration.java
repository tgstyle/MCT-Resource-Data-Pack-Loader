...
public class DeepGeneration {
    ...
    static final double CAVE_THRESHOLD = 0.0D;
    ...
    public DeepGeneration(World world, int offsetBlocks, IBlockState extensionBottom) {
        ...
        this.sky = new DeepSky(this, world, offsetBlocks);
        ...
        ContentLog.LOGGER.info("Deep and sky settings for dimension {}: deepStone={} noiseCaves={} deepRavines={} oreVeins={} skyStone={} skyShape={}",
                ...
                sky.skyStone == null ? "none" : sky.skyStone.getBlock().getRegistryName(), sky.skyShape == DeepSky.SKY_CAVES ? "caves" : "islands");
    }

    String scoped(String key, String fallback) {
    ...
    public boolean wantsSky() { return sky.skyStone != null; }

    public void fillSkyCube(CubePrimer primer, int cubeX, int cubeY, int cubeZ, Random rand, boolean topBedrock, boolean bottomBedrock) { sky.fillSkyCube(primer, cubeX, cubeY, cubeZ, rand, topBedrock, bottomBedrock); }
    ...
    double[][][] sampleLattice(int worldX, int genY, int worldZ, int ySteps) {
    ...
    double trilerp(double[][][] lattice, int x, int y, int z) {
        int x0 = x >> 2;
        int y0 = y >> 3;
        int z0 = z >> 2;
        double fx = (x & 3) / 4.0D;
    ...
    IBlockState veinState(int x, int genY, int z, IBlockState base) {
        for (Vein vein : veins) {
