...
public final class Commands {
    ...
    private static String particleName(String given, int[] extra) {
        ...
        return switch (given) {
            case "mobSpell" -> "minecraft:entity_effect{color:-1}";
            case "mobSpellAmbient" -> "minecraft:entity_effect{color:654311423}";
            case "reddust" -> "minecraft:dust{color:[1.0f,0.0f,0.0f],scale:1.0f}";
            case "barrier" -> "minecraft:block_marker{block_state:\"minecraft:barrier\"}";
            case "iconcrack" -> {
                ...
                yield "minecraft:item{item:\"" + item + "\"}";
            }
            case "blockcrack", "blockdust", "fallingdust" -> {
                ...
                block.properties().forEach((key, value) -> properties.add(key + ":\"" + value + "\""));
                yield ("fallingdust".equals(given) ? "minecraft:falling_dust" : "minecraft:block") + "{block_state:{Name:\"" + block.name() + "\",Properties:{" + String.join(",", properties) + "}}}";
            }
