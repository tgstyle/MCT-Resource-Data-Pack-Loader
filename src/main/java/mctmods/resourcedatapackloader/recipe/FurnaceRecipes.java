...
public final class FurnaceRecipes {
    ...
    public static void reload() {
        ...
        for (Map.Entry<String, int[]> entry : perPack.entrySet()) {
            ContentLog.LOGGER.info("  {} removed {} and added {}", entry.getKey(), entry.getValue()[0], entry.getValue()[1]);
        }
    }
    ...
    private static void apply(ResourceLocation key, String contents, int[] counts) {
        ...
        if (json.has("remove")) {
            for (JsonElement element : JsonUtils.getJsonArray(json, "remove")) {
                counts[0] += remove(key, element);
            }
        }
